...
def main(save_sample, save_categories):
    ...
    sample_cat2reviews_low={}
    sample_cat2reviews_high={}
    sample_cat2ratings={}
    num_reviews = 0
    with open (path2reviews, 'r') as f:
        for line in f.readlines():
            ...
            if rid in sample_rid2cat:
                for rcat in sample_rid2cat [ rid ]:
                    ...
                    if review_json['stars'] in [1,2]:
                        if rcat in sample_cat2reviews_low:
                            sample_cat2reviews_low [ rcat ].append(review_json['text'])
                        else:
                            sample_cat2reviews_low [ rcat ] = [review_json['text']]        
                    else:
                        if rcat in sample_cat2reviews_high:
                            sample_cat2reviews_high [ rcat ].append(review_json['text'])
                        else:
                            sample_cat2reviews_high [ rcat ] = [review_json['text']]


    if save_sample:  
        print "sampling restaurant reviews - low stars"
        ...
        count = 0
        max_bound = 0
        for cat in sample_cat2reviews_low:
            print cat
            new_max_bound = max_bound + len(sample_cat2reviews_low[cat])
            while count < sample_size and sorted_rev_sample[count] < new_max_bound:
                my_sample_v2.append( sample_cat2reviews_low[cat][ sorted_rev_sample[count] - max_bound ].replace("\n", " ").strip() )
                count = count + 1
        ...
        count = 0
        max_bound = 0
        for cat in sample_cat2reviews_high:
            print cat
            new_max_bound = max_bound + len(sample_cat2reviews_high[cat])
            while count < sample_size and sorted_rev_sample[count] < new_max_bound:
                my_sample_v3.append( sample_cat2reviews_high[cat][ sorted_rev_sample[count] - max_bound ].replace("\n", " ").strip() )
                count = count + 1
            max_bound = new_max_bound            


        with open ("review_sample_low_rating_100000.txt", 'w') as f:
            f.write('\n'.join(my_sample_v2).encode('ascii','ignore') )
            
        with open ("review_sample_high_rating_100000.txt", 'w') as f:
            f.write('\n'.join(my_sample_v3).encode('ascii','ignore') )
